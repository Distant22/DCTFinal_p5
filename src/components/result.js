  ...
  return (
    <div className={`duration-300 flex flex-col h-full w-screen items-center justify-center`}>
      ...
      <p className='h-[73%] w-[90%] flex flex-col overflow-y-scroll my-2'>
        ...
        <div className="chat chat-start">
          <div className='chat-bubble duration-300 bg-gray-200 text-gray-800'>可以點擊「重新開始」再玩一次，或是點擊「看結果」取得結果圖片</div>    
        </div>
      ...
      <p className={` ${ imageUrl !== "" ? "opacity-100" : "-translate-y-12 opacity-0" } relative duration-700 w-full h-[6%]  mb-2 space-x-3 flex justify-center`}>
        <button onClick={() => handleSendText("Restart")} className="w-[25%] h-[90%] rounded-full bg-gray-500 text-gray-200">重新開始</button>
        <button onClick={() => handleSendText("Result")} className="w-[25%] h-[90%] rounded-full bg-green-400 ">看結果</button>
      </p>
